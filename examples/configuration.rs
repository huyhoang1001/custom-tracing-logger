...
fn sample_operation(operation_id: u64) {
    ...
    warn!(operation_id = operation_id, "Operation completed with warnings");
}
...
fn main() {
    ...
    println!("Debug level:                $env:RUST_LOG='debug'; cargo run --example configuration");
    println!("File logging:               $env:LOG_FILE_DIR='./logs'; cargo run --example configuration");
